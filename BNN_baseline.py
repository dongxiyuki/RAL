...
class MLP_BBB(nn.Module):
    def __init__(self, input_dim, hidden_units, noise_tol=.1,  prior_var=1.):
    ...
    def sample_elbo(self, input, target, samples):
        ...
        for i in range(samples):
            outputs[i] = self.forward(input).reshape(-1) # make predictions
...
for i in range(epoch):
    ...
    for batch_id, batch_data in enumerate(loader):
        ...
        optimizer.zero_grad()
        loss = net.sample_elbo(batch_x, batch_y, 1)
